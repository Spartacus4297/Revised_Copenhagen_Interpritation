...
import logging
# Logging set up
logging.basicConfig(level=logging.DEBUG, filename='debug.log', filemode='a', format='%(asctime)s - %(levelname)s - %(message)s')
# Metadata Dictionary.
metadata = {
    "Timestamp": datetime.now().isoformat(),
    "Script Version": "0.1",
    "Description": " Cosmological data calculations and validation.",
    "Author(s)": "Mr. Orry Kneale"
}
...
Regions_data = {
    ...
    'Volume_diameter': [7.56858438e+23, 1.892146095e+24, 8.799993e+17],    # Diameter of the volume of the region/scale in meters.
}
...
async def calculate_CMB_scale_factor():
    # Observational data.
    Z = observational_data['WMAP_redshift']
    ...
    # Return
    return a_t
async def calculate_OU_scale_factor():
    # Observational data.
    Z = 0
    # Scaling factor calculation.
    a_t = 1 / (1 + Z)
    ...
    return a_t
# Lambda Calculations.
async def calculate_CMB_Lambda():
    # Constants.
    h = constants_dict['h']
    # Calculation Dependencies.
    E_H_0_CMB = await H_0_CMB_energy_conversion()
    ...
    t_hubble = await calculate_CMB_time()
    a_t = await calculate_CMB_scale_factor()
    # t_hubble energy conversion.
    E_t_hubble = t_hubble / h
    # Lambda calculation.
    L = (E_H_0_CMB * (Rho_E_x / E_t_hubble)) / a_t
    # Debug Logging.
    logging.debug(f"Calculated Lambda : {L}")
    # Return.
    return L
async def calculate_OU_Lambda():
    # Constants.
    h = constants_dict['h']
    # Calculation Dependencies.
    E_H_0_CMB = await H_0_CMB_energy_conversion()
    Rho_E_x = await calculate_CMB_Rho_E_x()
    t_hubble = await calculate_CMB_time()
    a_t = await calculate_OU_scale_factor()
    # t_hubble energy conversion.
    E_t_hubble = t_hubble / h
    # Lambda calculation.
    L = (E_H_0_CMB * (Rho_E_x / E_t_hubble)) / a_t
...
async def calculate_universal_Rho_E():
    ...
    # Return
    return Rho_E
# Saving Calculation data to JSON.
def save_it(filename, calculated_data, metadata):
    """
    Saving Debug log and Calculated data to Json file
    
    parameter: filename (str): Name of the JSON file to save data.

    parameter: calculated_data (dict): Dictionary of calculated Values.
    """
    try:
        # Load debug content.
        with open('debug.log', 'r') as log_file:
            debug_logs = log_file.readlines()
...
async def main():
    ...
    CMB_Rho_E_x = await calculate_CMB_Rho_E_x()
    CMB_Lambda = await calculate_CMB_Lambda()
    OU_Lambda = await calculate_OU_Lambda()
    ...
    print(f"Calculated value of Lambda for the CMB emmission Epoch: {CMB_Lambda}")
    print(f"Calculated value of Lambda for the Observable Universe: {OU_Lambda}")
    ...
    # Calculated data dictionary.
    caclulated_data = {
        "Calculated Rho_E (x) of CMB": CMB_Rho_E_x,
        "Calculated value of Lambda for the CMB emmission Epoch": CMB_Lambda,
        "Calculated value of Lambda for the Observable Universe": OU_Lambda,
        "Calculated radiation energy density (Rho_rad)": Rho_rad,
        "Calculated Delta Rho_E (x)": Delta_Rho_E_x,
        "Calculated Rho_E (x) Percentage difference": CMB_Rho_E_x_percentage_difference,
        "Calculated Rho_E (x) Ratio": CMB_Rho_E_x_ratio,
        "Calculated Universal Rho_E": Rho_E,
        "Calculated standard Model Energy density": Rho_total_standard,
        "Rho_E percentage difference": Rho_E_percentage_difference,
        "Calculated Rho_E ratio": Rho_E_ratio,
    }
    # Saving Calculated results.
    save_it("results.json", caclulated_data, metadata)
